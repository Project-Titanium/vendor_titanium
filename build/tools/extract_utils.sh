...
function setup_vendor() {
    ...
    export PTI_ROOT="$3"
    if [ ! -d "$PTI_ROOT" ]; then
        echo "\$PTI_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$PTI_ROOT/$OUTDIR" ]; then
        mkdir -p "$PTI_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$PTI_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$PTI_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$PTI_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PTI_ROOT"/vendor/pti/build/tools/smali/baksmali.jar
        export SMALIJAR="$PTI_ROOT"/vendor/pti/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$PTI_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$PTI_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$PTI_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$PTI_ROOT"/vendor/pti/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$PTI_ROOT"/"$OUTDIR"/radio
