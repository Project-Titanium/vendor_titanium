...
func init() {
	android.RegisterModuleType("pti_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ptiExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ptiExpandVariables(ctx, dep_file)
	...
	cmd := ptiExpandVariables(ctx, String(g.properties.Cmd))
